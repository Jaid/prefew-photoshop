...
const {default: prefewPhotoshop, parallel} = require(indexModule)

it("should run with 1 argument", () => {
  const result = prefewPhotoshop(["a", "b"])
...
it("should run with an integer as second argument", () => {
  const result = prefewPhotoshop(["a", "b"], 7)
  expect(result).toEqual({
    a: 7,
    b: 7,
  })
...
it("should run with a function as second argument", () => {
  ...
  const result = prefewPhotoshop(["a", "b"], valueGenerator)
  expect(result).toEqual({
    a: "1-a-x",
    b: "2-b-x",
  })
